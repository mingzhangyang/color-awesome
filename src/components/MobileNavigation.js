  ...
  getNavHTML() {
    const navItems = [
      ...
      },
      {
        ...
        label: 'Tools',
        icon: `<svg class="mobile-nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.5 6.5l3 3-7.5 7.5H7v-3l7.5-7.5z"/>
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 8l3 3"/>
    ...
    return navItems.map(item => `
      ...
      >
        ${item.icon}
        <span class="mobile-nav-label mt-1">${item.label}</span>
      </button>
    `).join('')
  }
